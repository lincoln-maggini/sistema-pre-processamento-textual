...
from models.renomearProcessos_model import renomear_processo
import jwt

SECRET_KEY = "matheuselincoln123"

def renomear_processo_controller():
    try:
        ...
        if not processo_id or not novo_nome:
            return jsonify({"success": False, "error": "Dados incompletos."}), 400

        sucesso = renomear_processo(processo_id, novo_nome, usuario_id)

        if sucesso:
            return jsonify({"success": True, "message": "Processo renomeado com sucesso."}), 200
        else:
